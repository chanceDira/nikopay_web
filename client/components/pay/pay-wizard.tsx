...
export function PayWizard() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const method = localStorage.getItem("nikopay_auth_method") || "wallet";
      const emailOk = localStorage.getItem("nikopay_email_verified") === "true";
      const walletOk =
        localStorage.getItem("nikopay_wallet_connected") === "true";
      const activeWallet =
        localStorage.getItem("nikopay_wallet_name") || "MetaMask";
      const storedRate = localStorage.getItem("nikopay_fx_rate");
      const storedFee = localStorage.getItem("nikopay_fx_fee");

      if (storedRate) setRate(parseFloat(storedRate));
      if (storedFee) setFeePercent(parseFloat(storedFee));

      setAuthMethod(method);
      setEmailVerified(emailOk);
      setModalEmail("");
      setIsWalletConnected(walletOk);
      setWalletName(activeWallet);
