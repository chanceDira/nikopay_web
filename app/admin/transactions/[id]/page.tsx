...
export default function AdminTransactionDetailPage({
  ...
  const { id } = use(params);
  const loaded = getMockIntent(id) ?? null;
  const [intent, setIntent] = useState<PaymentIntent | null>(loaded);
  const [editTxHash, setEditTxHash] = useState(loaded?.depositTx || "");
  const [editMomoRef, setEditMomoRef] = useState(loaded?.momoRef || "");
