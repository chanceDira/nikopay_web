...
import {
  fetchDepositCorridorCountries,
  fetchDepositCorridorProviders,
...
export function AdminCollections() {
  ...
  const loadRows = async () => {
    ...
    const json = (await res.json()) as {
      data?: { collections: CollectionView[] };
      error?: string;
    };
    if (!res.ok) {
      setErrorMsg(json.error ?? "unable to load collections");
      return;
    }
    setRows(json.data?.collections ?? []);
  };
  ...
  const loadProviders = async (nextCountry: string, preferred?: string) => {
    const result = await fetchDepositCorridorProviders(nextCountry);
    if (!result.ok) {
      applyProviders([]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      void loadRows();
      const countriesResult = await fetchDepositCorridorCountries();
      if (cancelled) {
        return;
      }
      ...
      if (!preferred) {
        setCorridorLoading(false);
        setErrorMsg("no deposit countries configured");
        return;
      }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSaving(false);
    if (!res.ok) {
      setErrorMsg(json.error ?? "unable to start collection");
      await loadRows();
      return;
    }
    setLabel("");
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="flex items-end">
          ...
          >
            {saving ? "Requesting..." : "Request collection"}
          </button>
        </div>
      </form>
