...
export function AdminFxForm() {
  ...
  const [minUsdt, setMinUsdt] = useState(() =>
    parseFloat(readLocal("nikopay_fx_min_usdt", "10")),
  );
  ...
  const [history, setHistory] = useState<RateLog[]>(() => {
    ...
    if (storedHistory) {
      return JSON.parse(storedHistory) as RateLog[];
    }
    localStorage.setItem("nikopay_fx_history", JSON.stringify(SEED_HISTORY));
    return SEED_HISTORY;
  });
