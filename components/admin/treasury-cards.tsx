...
export function AdminTreasuryCards() {
  const [momoBalance, setMomoBalance] = useState(() =>
    parseInt(readLocal("nikopay_momo_balance", "4500000"), 10),
  );
  const [polygonBalance] = useState(() =>
    parseFloat(readLocal("nikopay_polygon_balance", "25000")),
  );
  const [baseBalance] = useState(() =>
    parseFloat(readLocal("nikopay_base_balance", "18500")),
  );
